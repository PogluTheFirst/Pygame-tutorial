...
class Player(AnimatedSprite):
    ...
    def input(self):
        ...
        if keys[pygame.K_w] and not self.shoot_timer:
    ...
    def animate(self, dt):
        if self.direction.x: #? if player moving right or left
            self.frame_index += self.animation_speed * dt
            self.flip = self.direction.x < 0 #? returns bool depending on direction
        else:
            self.frame_index = 0
